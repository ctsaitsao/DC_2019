...
def draw():   # In Pygame Zero, there's no main() and no functions called. It automatically calls draw() and update() 30 times a second
    screen.fill((0, c, 0))  # RGB
    plot(x,10,HEIGHT/2,0,0,255) # data, xpos, ypos, red, green, blue
    screen.draw.text(str(distance), (360, 20), color="orange", fontsize=80) # top left to bottom right, ypos is inverted
    screen.draw.text(str(light), (0, 20), color="orange", fontsize=80)
    ### Draw arena
